...
- [ ] **1.1: Define `ENABLE_MULTI_RELEASE` Flag**
...
- [ ] **2.2: Adapt Publish Job for Multi-Release**
...
- [ ] **3.1: Implement Release Logic**
    - **Action:** The script will read the `ENABLE_MULTI_RELEASE` variable (available in the CI environment).
        - If `false` or unset, it will simply execute `changeset publish`.
        - If `true`, it will perform pre-flight checks:
            1.  **If on `main`:** Check the `.changeset` directory for files indicating a `major` version bump. If found, verify that a maintenance branch (e.g., `release/<package>_<major-1>`) already exists. If not, fail the workflow with a clear error message instructing the user to create the branch manually.
            2.  **If on a `release/*` branch:** Check for `major` version changesets and fail if any are found, as maintenance branches should only be for patch and minor releases.
    - **Action:** After checks pass, the script will run `changeset publish`.
